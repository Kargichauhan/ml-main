...
param_dist = {
    ...
    'model__neurons': [64, 32, 16,128, 64, 32, 256, 128, 64,256, 128, 64,256, 128],
    'model__layers': [1, 2, 3],
...
model = KerasRegressor(model=create_model, verbose=1, epochs=50)  # Adjust epochs as needed
random_search = RandomizedSearchCV(estimator=model, param_distributions=param_dist, n_iter=450, n_jobs=-1, cv=3, random_state=42)  # Reduced n_iter for example

random_search_result = random_search.fit(X_train, y_train)

#save to csv 
result_df = pd.DataFrame(random_search_result.cv_results_)
result_df.to_csv('random_search_results1.csv',index=False)
